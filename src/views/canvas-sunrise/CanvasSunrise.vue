  ...
  data() {
    return {
      ...
      drawingList: [
        ...
        {
          mode: DRAWING_MODE.cloud_type1,
          x: 350,
          y: 0,
          scaleX: 0.45,
          scaleY: 0.4,
          color: "rgba(97, 185, 255,0.9)",
          motionIndex: 0,
          motionSpeed: 250,
          motionRange: 70,
        },
    ...
    sun(params) {
      ...
      for (let i = 1; i <= BNUMBER; i += 6) {
        ...
        this.ctx.arc(
          ...center,
          radius,
          ((2 * Math.PI) / BNUMBER) * (i + params.motionIndex / 15),
          ((2 * Math.PI) / BNUMBER) * (i + params.motionIndex / 15 + 2)
        );
        this.ctx.fill();
      }
      //
